...
const DashboardVendedor = () => {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      ...
      try {
        ...
        // Calcular ventas de hoy comparando fecha en formato YYYY-MM-DD (evita desfases por TZ)
        const obtenerClaveFecha = (fechaEntrada) => {
          if (!fechaEntrada) return '';
          if (typeof fechaEntrada === 'string' && fechaEntrada.length >= 10) {
            return fechaEntrada.slice(0, 10); // asume formato 'YYYY-MM-DD ...'
          }
        ...
        const ventasHoy = ventas.filter(v => obtenerClaveFecha(v.fecha) === hoyClave);
        
        const ventasHoyTotalCalculado = ventasHoy.reduce((sum, v) => sum + parseFloat(v.total || 0), 0);

        // Procesar ventas recientes solo con ventas de hoy; si no hay, mostrar vacío
        // Ventas recientes del día: filtrar por fecha de hoy y deduplicar clientes (máx 5)
        const ventasHoyParaLista = ventas
          .filter(v => obtenerClaveFecha(v.fecha) === hoyClave)
          .sort((a, b) => new Date(b.fecha) - new Date(a.fecha)); // más recientes primero

        const clientesUnicosLista = new Set();
        const ventasRecientes = [];

        for (const venta of ventasHoyParaLista) {
          const claveCliente = venta.id_cliente
            ? `id_${venta.id_cliente}`
            : (venta.nombre_cliente || venta.cliente || 'cliente_general');

          if (!clientesUnicosLista.has(claveCliente)) {
            clientesUnicosLista.add(claveCliente);
            const fecha = new Date(venta.fecha);
            ventasRecientes.push({
              id: venta.id_venta,
              cliente: venta.nombre_cliente || venta.cliente || 'Cliente General',
          ...
          if (ventasRecientes.length >= 5) break;
        }
  ...
  return (
    <div className="dashboard-container">
      ...
      {/* Sección de ventas recientes (solo del día) */}
